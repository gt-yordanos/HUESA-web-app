...
const EditMember = ({ member, onClose, onSubmit }) => {
  ...
  const checkExistingUser = async () => {
    ...
    if (!emailSnapshot.empty && emailSnapshot.docs[0].id !== member.id) {
      toast.error('Already registered with this email.');
      return true;
    }
    if (!phoneSnapshot.empty && phoneSnapshot.docs[0].id !== member.id) {
      toast.error('Already registered with this phone number.');
      return true;
    }
    if (!idSnapshot.empty && idSnapshot.docs[0].id !== member.id) {
    ...
    <div className="modal modal-open">
      <div className="modal-box">
        ...
        <form onSubmit={handleSubmit}>
          {/* Form Fields (same as AddMember, but pre-filled) */}
          <input
            name="studentId"
          ...
          <button type="submit" className=  {`btn w-full text-white ${loading ? "btn-base-200" : "btn-info"}`}>
            {loading ? <span className="loading loading-spinner text-info"></span> : 'Update Member'}
          </button>
        </form>
